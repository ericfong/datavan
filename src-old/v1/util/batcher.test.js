...
import { batcher, makeBatchIdQuery, joinMiddlewares } from './batcher'
...
test('batcher', async () => {
  const batch = batcher(argsArr => Promise.resolve(argsArr.map(([id]) => `Echo-${id}`)))
  const rets = await Promise.all([batch('1', { load: true }), batch('2', { load: true }), batch('3', { load: true })])
