...
test('basic', async () => {
        ...
        findFetch(query) {
          ...
          if (query) {
            if (query.$search) {
              ...
              ++calledGet
              const ids = getQueryIds(query)
              // console.log('findFetch get', ids, calledGet)
  ...
  await store.getPromise()
  expect( store.users.find(null, {sort: {_id: 1}}) ).toEqual([{_id: 'u1', name: 'u1 name'}, {_id: 'u2', name: 'users Eric'}, {_id: 'u3', name: 'hi Simon'}])
